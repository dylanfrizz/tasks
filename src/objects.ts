...
export function isCorrect(question: Question, answer: string): boolean {
    //let trimQuestion: string = question.expected.trim();
    return (
...
export function isValid(question: Question, answer: string): boolean {
    const sameAnswers = question.options.filter(
        (stringg: string): boolean => stringg === answer,
    );
...
export function toShortForm(question: Question): string {
    let sForm: string = "";
    sForm += question.id.toString() + ": ";
    if (question.name.length <= 10) {
        sForm = sForm + question.name;
    } else {
        sForm = sForm + question.name.slice(0, 10);
    }
    return sForm;
}
...
export function toMarkdown(question: Question): string {
    ...
    if (question.type === "multiple_choice_question") {
        markdown += "\n";
        const optionss = question.options.map(
            (stringg: string): string => "- " + stringg + "\n",
        );
        const fullString = optionss.reduce(
            (full: string, stringg: string) => full + stringg,
            "",
        );
        const noLastIndent: string = fullString.slice(0, -1);
        markdown += noLastIndent;
    }
    return markdown;
}
